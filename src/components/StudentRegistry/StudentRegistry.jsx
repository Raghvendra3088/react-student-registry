...
function AddStudentForm({ onAdd, onClose }) {
  ...
  return (
    <div style={{
      ...
      boxShadow: "0 8px 40px rgba(0,0,0,0.12)", padding: 24,
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
        <label style={{ fontSize: 11, fontWeight: 600, color: "#6b7280", letterSpacing: "0.06em", textTransform: "uppercase" }}>Course</label>
        <select
          value={form.course}
      ...
      <button
        ...
        style={{
          ...
          boxShadow: "0 4px 12px rgba(99,102,241,0.35)", transition: "transform 0.15s",
        }}
      >Add Student</button>
    </div>
  );
      ...
      <div style={{
        ...
        <div style={{ width: "100%", maxWidth: 560 }}>
          <div style={{ marginBottom: 28 }}>
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
              <div>
                ...
                <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 32, fontWeight: 900, color: "#111", lineHeight: 1.1, letterSpacing: "-0.02em" }}>
                  Student<br /><span style={{ color: "#6366f1" }}>Registry</span>
                </h1>
              </div>
              <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6 }}>
                <div style={{ background: "#fff", borderRadius: 10, padding: "8px 14px", border: "1px solid #f0f0f0", textAlign: "center" }}>
                  <div style={{ fontSize: 22, fontWeight: 800, color: "#6366f1", fontFamily: "'Playfair Display', serif" }}>{students.length}</div>
                  <div style={{ fontSize: 10, color: "#9ca3af", fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase" }}>Students</div>
                </div>
                <div style={{ background: "#fff", borderRadius: 10, padding: "6px 14px", border: "1px solid #f0f0f0", textAlign: "center" }}>
          ...
          <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
            ...
            >{showForm ? "Cancel" : "+ Add Student"}</button>
          </div>
          ...
          <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #f0f0f0", boxShadow: "0 2px 12px rgba(0,0,0,0.06)", overflow: "hidden" }}>
            <div style={{ padding: "12px 18px", borderBottom: "1px solid #f5f5f5", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: 11, fontWeight: 700, color: "#9ca3af", letterSpacing: "0.1em", textTransform: "uppercase" }}>
                {filtered.length} {filtered.length === 1 ? "record" : "records"}{search && ` for "${search}"`}
              </span>
              {search && <button onClick={() => setSearch("")} style={{ fontSize: 11, color: "#6366f1", background: "none", border: "none", cursor: "pointer", fontWeight: 600 }}>Clear</button>}
            </div>
            <div style={{ padding: "10px", display: "flex", flexDirection: "column", gap: 6, maxHeight: 420, overflowY: "auto" }}>
              {filtered.length === 0 ? (
                ...
                filtered.map((s, i) => <StudentCard key={s.id} student={s} onDelete={handleDelete} index={i} />)
              )}
            </div>
          </div>
